<template>
  <div>
    <!--封面-->
    <dl class="music-info" :style="bigwidth?'display:none;':''">
      <dt @click="showLyric()">
        <img :src="musicPicUrl" />
      </dt>
      ...
      <template v-else>
        <dd>小康在线音乐播放器</dd>
        <dd>
          <a class="github" target="_blank" href="https://github.com/xkloveme">superDragon</a>
        </dd>
      </template>
    </dl>
    <!--歌词-->
    <div class="music-lyric" :style="bigwidth?'top:0px;':''" ref="musicLyric" @click="showLyric()">
      <div :class="bigwidth?'music-lyric-items1':'music-lyric-items'" :style="lyricTop">
        <p v-if="!currentMusic.id">还没有播放音乐哦！</p>
        <p v-else-if="nolyric">暂无歌词！</p>
        <template v-else-if="lyric.length>0" :style="bigwidth?'font-size:24px;':''">
